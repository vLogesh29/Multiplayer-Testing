...
public class PredictionMotor : NetworkBehaviour
{
    ...
    private void TimeManager_OnUpdate()
    {
        if (base.IsOwner)
        {
            inputValues = _input.GetInputValues();
    ...
    private void CameraRotation()
    {
        ...
        if (inputValues.look.sqrMagnitude >= _threshold && !LockCameraPosition)
        {
            //Don't multiply mouse input by Time.deltaTime;
            float deltaTimeMultiplier = IsCurrentDeviceMouse ? 1.0f : Time.deltaTime;

            _cinemachineTargetYaw += inputValues.look.x * deltaTimeMultiplier;
            _cinemachineTargetPitch += inputValues.look.y * deltaTimeMultiplier;
        }
