...
{
    public class ThirdPersonControllerMirrorBehaviour : NetworkBehaviour, ThirdPersonController
    { 
        [Header("Player")]
        ...
        {
            Debug.Log("Client Player["+netId+"] given authority! "+isOwned);
            gameObject.name += "-netID:[" + netId + "]";
            if (isOwned)
        ...
        private void Update()
        {
            if (!isOwned && !(_isBot && isServer)) return;
